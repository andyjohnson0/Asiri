...
{
    /// <summary>
    /// Decrypts data in XTS mode (IEEE P1619), given already-initialized data and tweak block
    /// ciphers. BouncyCastle.Cryptography does not include an XTS mode implementation, so the tweak
    /// generation and GF(2^128) chaining defined by the XTS specification are implemented here, once
    /// - the chaining itself is identical regardless of which underlying block cipher XTS is applied
    /// to, so it is shared rather than duplicated across the Xts*Cipher classes. Those classes are
    /// responsible only for selecting and initializing the underlying cipher (see
    /// <see cref="XtsAesCipher"/>, <see cref="XtsSerpentCipher"/>).
    /// </summary>
    internal static class XtsBlockCipher
    {
        ...
        public static byte[] Decrypt(IBlockCipher dataCipher, IBlockCipher tweakCipher, byte[] cipherText, long dataUnitNumber)
        {
            ...
            {
                throw new ArgumentException("Cipher text length must be a multiple of the cipher's block size.", nameof(cipherText));
            }
            ...
            for (var i = 0; i < blockCount; i++)
            {
                ...
                {
                    block[j] = (byte)(cipherText[offset + j] ^ t[j]);
                }
